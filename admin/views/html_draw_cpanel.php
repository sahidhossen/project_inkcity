 ...
 */
	...
	$markers_list = '';
	 $dir = dirname(dirname(__FILE__)).'/assets/graphics/markers';
//	 $dir = admin_home().'/assets/graphics/markers';
            ...
            if ($dh = opendir($dir)) {
                ...
                while (false !== ($file = readdir($dh))) {
                    ...
                    if (in_array($extension, $allowedExts))	{
                            $markers_list .= '<img src="'.admin_stylesheet().'graphics/markers/'.$file.'" id="'.$file.'" class="marker_list"  onClick="setIcon($(this).attr(\'id\'));" />&nbsp;';
                    }
                }
                closedir($dh);
            }
	...
	// set default marker
	$default_marker = admin_stylesheet().'graphics/markers/default.png';
	...
	<div id="markers_list_content" style="display:none;">
		...
		<script>
			function setIcon(myvalue) {
				...
				if (what) {
					map.markers[what].setIcon('<?php echo admin_stylesheet() ?>graphics/markers/'+myvalue);
					$('#marker_icon_'+what).val('<?php echo admin_stylesheet() ?>graphics/markers/'+myvalue);
					$('#marker_'+what+'_chm').popover('hide');
				}
				else {
					$('#current_marker').html('<img src="<?php echo admin_stylesheet() ?>graphics/markers/'+myvalue+'" alt="Current marker" />');
					$('#current_marker_path').html('<?php echo admin_stylesheet() ?>graphics/markers/'+myvalue);
