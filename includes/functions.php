...
function html_map_options($map_id, $lang_msg, $global = false) {
	...
	if (!$global) { // single map view
		...
		$map_options .= '<a type="button" class="btn btn-info btn-xs" id="full_map_'.$map_id.'" href="'.admin_url('maps').'?p=map_view&map_id='.$map_id.'&map_o=1" target="_blank">'.$lang_msg['MAPS_FULL_VIEW'].'</a>';
	}
	else {	// global map view
		$map_options .= '<a type="button" class="btn btn-info btn-xs" id="full_map_global" href="'.admin_url('maps').'?p=map_global&map_o=1" target="_blank">'.$lang_msg['MAPS_FULL_VIEW'].'</a>';
	}
